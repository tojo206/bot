	...
	function loadData(url){
		...
		async function getResponse(prompt) {
			...
			if (use_php_api) {
			  ...
			  const pmt = continuous_chat
				? `The following is a conversation between ${character_name}: and User: \n\n ${conversations}`
				: prompt;
		  
			  // Use GPT-4 model in the OpenAI API
			  fetch("https://api.openai.com/v1/chat/completions", {
				...
				.then(data => {
				  if (data.choices && data.choices.length > 0 && data.choices[0].message.content) {
					responseChat(data.choices[0].message.content);
				  } else {
